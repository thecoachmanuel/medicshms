...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [
      ...
      (supabaseAdmin || supabase).from('public_appointments').select('*', { count: 'exact', head: true }).eq('hospital_id', profile?.hospital_id).eq('doctor_assigned_id', doctorId).gte('appointment_date', startOfToday).lte('appointment_date', endOfToday).in('appointment_status', ['Arrived', 'Triaged']),
      ...
      (supabaseAdmin || supabase).from('public_appointments').select('*', { count: 'exact', head: true }).eq('hospital_id', profile?.hospital_id).eq('doctor_assigned_id', doctorId).gte('appointment_date', startOfToday).lte('appointment_date', endOfToday).in('appointment_status', ['Confirmed', 'Arrived', 'Triaged']),
