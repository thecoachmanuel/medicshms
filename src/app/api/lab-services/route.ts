...
export async function GET(request: Request) {
  const { error: authError, profile, supabase: supabaseClient } = await withAuth(request, ['Lab Scientist', 'Doctor', 'Admin', 'Receptionist', 'Nurse', 'Radiologist', 'Pharmacist', 'Patient']);
  ...
  try {
    ...
    if (patientId) {
      if (profile.role === 'Patient') {
        const { data: pRecord } = await supabaseClient.from('patients').select('id').eq('user_id', profile.id).maybeSingle();
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (!request_id) {
      return NextResponse.json({ message: 'Request ID is required for updates' }, { status: 400 });
    }
    ...
    if (isScientist && !unit_id) {
      // Priority: Only auto-assign scientist's unit if the request doesn't already specify one
      // This protects requests from being moved between units during result entry
