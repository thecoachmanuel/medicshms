    ...
    <div className="relative min-h-[calc(100vh-10rem)] space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1 ml-15">Unified directory for investigation and analysis.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {loading ? (
          ...
          filteredPatients.map(patient => (
            <div key={patient.id} className="group relative bg-white/70 backdrop-blur-md border border-white/50 rounded-3xl p-6 shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-500 hover:-translate-y-1">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-blue-50 border border-blue-100 flex items-center justify-center font-black text-blue-600 text-lg shadow-inner">
                    {patient.full_name?.[0]}
                  </div>
                  <div>
                    <h3 className="font-black text-gray-900 text-lg leading-tight group-hover:text-blue-600 transition-colors uppercase tracking-tight">
                      {patient.full_name}
                    </h3>
                ...
                <Link 
                  href={`/${slug}/lab-scientist/requests?patientId=${patient.id}`}
                  className="w-10 h-10 rounded-xl bg-gray-900 border border-gray-800 flex items-center justify-center text-white hover:bg-blue-600 hover:scale-110 active:scale-95 transition-all shadow-lg"
                  title="Assign Test"
                >
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                 ...
                 <div className="bg-gray-50/50 p-3 rounded-2xl border border-gray-100/50">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1">Blood Group</p>
                    <p className="text-xs font-black text-rose-600 bg-rose-50 px-2 py-0.5 rounded-lg inline-block">{patient.blood_group || 'O+'}</p>
                 </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                 <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Stable Clinical Profile</span>
                 </div>
                 <Link 
                   href={`/${slug}/lab-scientist/requests?search=${patient.full_name}`}
                   className="flex items-center gap-2 text-[10px] font-black text-blue-600 uppercase tracking-widest hover:translate-x-1 transition-all"
                 >
