    ...
    <div className="fixed inset-y-0 right-0 left-0 lg:left-64 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white/95 backdrop-blur-md rounded-[3rem] max-w-2xl w-full shadow-[0_32px_128px_rgba(30,41,59,0.15)] border border-white/40 overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-10 no-scrollbar">
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
               <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold text-gray-900" value={enrollData.fullName} onChange={e => setEnrollData({...enrollData, fullName: e.target.value})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-1">Mobile Contact</label>
                    <input placeholder="+234..." className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold text-gray-900" value={enrollData.mobileNumber} onChange={e => setEnrollData({...enrollData, mobileNumber: e.target.value})} />
                  </div>
               </div>
               <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-1">Email (Optional)</label>
                    <input className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold text-gray-900" value={enrollData.emailAddress} onChange={e => setEnrollData({...enrollData, emailAddress: e.target.value})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-1">Biological Sex</label>
                    <select className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold text-gray-900 appearance-none" value={enrollData.gender} onChange={e => setEnrollData({...enrollData, gender: e.target.value})}>
               ...
               <div className="space-y-2">
                  ...
                  <input type="date" className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold text-gray-900" value={enrollData.dateOfBirth} onChange={e => setEnrollData({...enrollData, dateOfBirth: e.target.value})} />
               </div>
               <div className="flex gap-4 pt-4">
                  <button onClick={() => setStage('Discovery')} className="flex-1 py-4 text-xs font-black text-gray-400 uppercase" type="button">Discard</button>
                  <button onClick={handleEnrollPatient} disabled={loading} className="flex-[2] py-4 bg-gray-900 text-white rounded-2xl text-[10px] font-black uppercase flex items-center justify-center gap-3" type="button">
                    {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />} Create Identity
                  </button>
               </div>
            </div>
          )}
          ...
          {stage === 'Configure' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
              <div className="p-6 bg-blue-50/50 rounded-[2rem] border border-blue-100 flex items-center justify-between mr-1">
                <div className="flex items-center gap-4">
              ...
              <div className="space-y-4">
                ...
                <div className="relative group">
                  ...
                  <input placeholder="Search or Type new test..." className="w-full pl-14 pr-48 py-5 bg-gray-50/50 border border-gray-100 rounded-[1.5rem] focus:bg-white focus:ring-4 focus:ring-blue-500/10 outline-none font-black text-gray-900 shadow-inner"
                  ...
                  {isNewTest && (
                    <div className="absolute right-32 top-1/2 -translate-y-1/2 flex items-center gap-2 border-l border-amber-100 pl-4 h-8">
                      <span className="text-[9px] font-black text-amber-600 tracking-tighter">₦</span>
                      <input type="number" placeholder="Fee" required className="w-16 bg-transparent outline-none text-[10px] font-black text-emerald-600" id="manual-test-price" />
                    </div>
                  )}
                  <button onClick={() => {
              ...
              {selectedTests.length > 0 && (
                <div className="space-y-3 p-6 bg-gray-50/50 rounded-[2rem] border border-dashed border-gray-200">
                  <div className="flex flex-wrap gap-2">{selectedTests.map(t => (
                    <div key={t.test_name} className="flex items-center gap-2 pl-4 pr-2 py-2 bg-white border border-gray-100 rounded-xl shadow-sm">
                      <span className="text-xs font-black text-gray-700">{t.test_name}</span>
                      <button onClick={() => handleRemoveTest(t.test_name)} className="p-1 hover:bg-rose-50 rounded-lg text-gray-300 hover:text-rose-500" type="button"><X className="w-3.5 h-3.5" /></button>
                    </div>
                  ))}</div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input placeholder="Optional doctor's name..." className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-bold text-gray-900 focus:bg-white focus:ring-4 focus:ring-blue-500/10 transition-all font-bold" 
              ...
              <div className="space-y-2">
                ...
                <textarea rows={2} placeholder="Summary of patient status or indications..." className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-medium text-gray-700 resize-none" 
                  value={requestData.clinical_summary} onChange={e => setRequestData({...requestData, clinical_summary: e.target.value})} />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-1">Analytical Instructions (Lab Use)</label>
                <textarea rows={2} placeholder="Specific collection or handling notes..." className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none font-medium text-gray-700 resize-none" 
