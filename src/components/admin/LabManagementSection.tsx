...
export default function LabManagementSection() {
  ...
  const handleUpsertCatalog = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      description: data.get('description') as string,
      // Pass existing schema if editing to avoid stripping it
      template_schema: editingItem?.template_schema
    };
    try {
            ...
            {catalog.filter(i => i.test_name.toLowerCase().includes(searchTerm.toLowerCase())).map(item => (
              <div key={item.id} className="card p-5 group hover:border-emerald-200 transition-all relative overflow-hidden flex flex-col justify-between">
                ...
                <div className="flex justify-end gap-2 mt-6 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
                   <button onClick={() => { setEditingItem(item); setIsCatalogModalOpen(true); }} className="p-2.5 bg-gray-50 rounded-xl hover:text-emerald-600 hover:bg-emerald-50 transition-all"><Edit3 className="w-4 h-4" /></button>
      ...
      {isCatalogModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
           ...
           <div className="relative bg-white rounded-[3rem] w-full max-w-lg p-10 shadow-[0_32px_128px_rgba(30,41,59,0.2)] space-y-8 animate-in zoom-in-95 duration-300">
              ...
              <form onSubmit={handleUpsertCatalog} className="space-y-6">
